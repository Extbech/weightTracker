...
def update_weight(conn, name: str, date_obj: date, weight: float) -> None:
    ...
    try:
        print(f"inside sql query, values: {name}; {date_obj}, {type(date_obj)}; {weight}")
        cur = conn.cursor()
...
def select_max_weight(conn):
    ...
    cur = conn.cursor()
    cur.execute("SELECT name, timestamp, weight, max_weight_1, max_weight_2, max_weight_3, max_weight_4, max_weight_5, kcal_500, kcal_1000 FROM weightTracker")

    max_weights = []
    for (name, timestamp, weight, max_weight_1, max_weight_2, max_weight_3, max_weight_4, max_weight_5, kcal_500, kcal_1000) in cur:
        max_weights.append({ 
            "name": name, 
            "timestamp": timestamp,
